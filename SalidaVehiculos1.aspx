...
<script runat="server">
    ...
    Protected Sub Guardar_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        Dim consultaCupo As String
        Dim consultaAcumulado As String
        Dim cupo As String
        ...
        If Biblioteca.EjecutarSql(Mensaje, ssql) Then
            ...
            If Biblioteca.EjecutarSql(Mensaje, ssql) Then
                ...
                If Biblioteca.EjecutarSql(Mensaje, ssql) Then
                    ...
                    consultaCupo = "SELECT CUPOLIMITE FROM MINAS WHERE NUMERO='" & Biblioteca.ContrCoopMina(Trim(Me.NENTRADA.Text), 3) & "'"
                    consultaAcumulado = "select sum(entregas.pesoneto) as Acumulado from entregas, minas where entregas.mina=minas.numero and entregas.mina='" & Biblioteca.ContrCoopMina(Trim(Me.NENTRADA.Text), 3) & "'"
                   
                    ''Imprimir
                    'calcular cantidad suministrada
                    'consultar cupo maximo
                    conn = Biblioteca.Conectar(Mensaje)
                    
                    dTReader = Biblioteca.CargarDataReader(Mensaje, consultaCupo, conn)
        
                    If dTReader.Read() Then
                        cupo = dTReader("CUPOLIMITE")
                    End If
                    dTReader.Close()
                    
                    dTReader = Biblioteca.CargarDataReader(Mensaje, consultaAcumulado, conn)
                    If dTReader.Read() Then
                        acumulado = dTReader("Acumulado")
                    End If
                    
                    dTReader.Close()
                    conn.Close()
                    Dim cupoF = FormatNumber(cupo)
                    Dim acumuladoF = FormatNumber(acumulado)
                    Dim mensa = ("SU CUPO PROGRAMADO ES DE " & cupo & " Ton Y LA CANTIDAD ACUMULADA ES " & (acumulado/1000) & " Ton")
                   
                    Dim update = "UPDATE ENTREGAS SET OBSERVACION_SAL='" & mensa & _
                    "' WHERE ENTREGAS.NUMEROENTRADA = '" & Trim(Me.NENTRADA.Text) & "'"

                    
                    Biblioteca.EjecutarSql(Mensaje, update)
                    Biblioteca.SalidaVehiculo(Biblioteca.ContrCoopMina(Trim(Me.NENTRADA.Text), 3))
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">    
    ...
    <div>
    <table border="1" > <!-- TABLA USADA COMO MARCO DEL FORMULARIO-->
    <tr>
