...
<script runat="server">
    ...
    Protected Sub BtnActualizar_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If (RutaSubir.PostedFile.ContentLength > 0) Then
            ...
            Catch ex As Exception
                Cargado = False
                archivosplanos.GeneraArchivoBascula("c:\temp", "ErrorCargaProvee.txt", ex.Message)
    ...
    Private Function ImportarDesdeCsv(ByVal Ruta As String) As String
        ...
        ImportarDesdeCsv = 0
        Try
            ssql = "UPDATE MINAS SET ESTADO ='', KGS_ACUM = 0, CUPOLIMITE=0 "
            ...
            While Cadena <> ""
                ...
                Else
                    If ImportarDesdeCsv = 0 Then
                        ImportarDesdeCsv = "Se actualizaron " & Actualizados & " y se Insertaron " & Insertados & " Proveedores "
                    Else
                ...
                Proveedor = Vector(0)
                If Proveedor <> "" Then
                    'If Proveedor = "02/25/01" Then Stop

                    Descripcion = Vector(1)
                    Municipio = Vector(2)
                    If Municipio.Length = 1 Then
                        Municipio = "00" & Municipio
                    End If
                    If Municipio.Length = 2 Then
                        Municipio = "0" & Municipio
                    End If
                    
                    Unidad = Vector(4)
                    Estado = Vector(5)
                    if(Vector.Length>6 ) then
                    if( Vector(6)<>"") then 
                                 CupoLimite = Integer.Parse(Vector(6))
                                 end if
                                 
                     end if
                                 
                    ssql = "UPDATE MINAS" & _
                           " SET DESCRIPCION = '" & Descripcion & "', MUNICIPIO = '" & Municipio & "', OBSERVACIONES = '', UNIDAD = '" & Unidad & "', ESTADO = '" & Estado & "', ENTREGAS = 1, KGS_ACUM = 0, CUPOLIMITE=" & CupoLimite & _
                           " WHERE NUMERO = '" & Proveedor & "'"
                    ...
                    Else
                        ...
                        miDataReader.Close()
                        ssql = "INSERT INTO MINAS" & _
                               " (NUMERO, DESCRIPCION, MUNICIPIO, OBSERVACIONES, UNIDAD, ESTADO, CUPOLIMITE, ENTREGAS, KGS_ACUM) " & _
                               " VALUES ('" & Proveedor & "', '" & Descripcion & "', '" & Municipio & "', '', '" & Unidad & "', '" & Estado & "' , " & CupoLimite & ",1, 0)"
                        cmd = New SqlCommand(ssql, conn)
                        miDataReader = cmd.ExecuteReader()
                        If miDataReader.RecordsAffected > 0 Then
            ...
            End While
            ImportarDesdeCsv = "Se actualizaron " & Actualizados & " y se Insertaron " & Insertados & " Proveedores "
            Seguridad.RegistroAuditoria(Session("Usuario"), "Importar", "Proveedores", "Mensaje:" & Me.mensaje.Text & ";Ruta:" & Ruta, Session("GRUPOUS"))
            fileReader.Close()
        Catch ex As Exception
            fileReader.Close()
            ...
            <table style="width: 347px; height: 182px">
                <tr>
                    <td align="center" colspan="2" height="1" style="color: #000000">
                        <asp:Label ID="Label17" runat="server" Font-Bold="True" ForeColor="#336677" Text="Importar Proveedores" Width="179px"></asp:Label></td>
                </tr>
                <tr>
                    <td align="left" colspan="2" style="height: 24px; text-align: center">
                        <asp:Label ID="Label1" runat="server" Font-Bold="True" ForeColor="Red" Text="Se actualizaran todos los campos de la tabla proveedores, y se crearan los registros que no se encuentren actualmente"></asp:Label></td>
                </tr>
                <tr>
