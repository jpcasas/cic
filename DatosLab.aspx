<%@ Page Language="VB" MasterPageFile="~/MasterPage.master" Title="Análisis de Laboratorio Periodo Actual" %>
...
<script runat="server">
    ...
    private sub CargarGridView
        ...
        ssql = " SELECT MUESTRAS.NUMERO as [Cód Muestra], RIGHT('0' +LEFT(DAY(FECHAMUESTRA), 2),2) + '/' + RIGHT('0' + LEFT(MONTH(FECHAMUESTRA), 2), 2) + '/' + LEFT(YEAR(FECHAMUESTRA),4) AS [Fecha Análisis], " & _
               " MUESTRAS.HUMEDADRES AS [% Hum Res], MUESTRAS.HUMEDADSUP AS [%Hum Sup], MUESTRAS.CENIZAS AS [% Cenizas], MUESTRAS.MATVOLATIL AS [% Mat Volátil], MUESTRAS.AZUFRE AS [% Azufre], MUESTRAS.PODERCALORHHV AS [HHV Poder Calorífico], MUESTRAS.ANALISTA AS [Análista], MUESTRAS.ESTADO AS [Estado]" & _
               " FROM MUESTRAS" & _
        ...
        DtSet = New DataSet
        DtAdapter.Fill(DtSet, "MUESTRAS")
        'llenar datagrid
        Me.MUESTRAS.DataSource = DtSet.Tables("MUESTRAS").DefaultView
    ...
    Protected Sub ImportarExcel_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If (RutaSubir.PostedFile.ContentLength > 0) Then
            ...
            Catch ex As Exception
                Cargado = False
                archivosplanos.GeneraArchivoBascula("c:\temp", "ErrorCargaArchivoExcel.txt", ex.Message)
    ...
    Protected Sub Imprimir_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        'Session("TituloReporte") = "Listado de Muestras del Período"
        'Dim Biblioteca As New Biblioteca
        ...
        Session("SqlReporte") = ssql
        Session("NombreReporte") = "DatosLab.rpt"
        Session("Parametro") = "Fecha de Impresión " & Format(Today, "dd/MMM/yyyy")
        Session("NombreDataTable") = "MUESTRAS"
        Seguridad.RegistroAuditoria(Session("Usuario"), "Reportes", "MUESTRAS", "FechaIn:" & Me.FECHAIN.Text & ";FechaFin:" & Me.FECHAFIN.Text, Session("GRUPOUS"))
    End Sub
    ...
    Protected Sub Calcular_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        If Me.CTodo.Checked Then
            Mensaje = Calcular.CalcularMuestras("MUESTRAS", True, Format(CDate(Me.FECHAIN.Text), "yyyy-MM-dd 00:00:00"), Format(CDate(Me.FECHAFIN.Text), "yyyy-MM-dd 00:00:00"))
            Seguridad.RegistroAuditoria(Session("Usuario"), "Calcular", "Todo", "FechaIn:" & Me.FECHAIN.Text & ";FechaFin:" & Me.FECHAFIN.Text & ";Tabla:Muestras", Session("GRUPOUS"))
        Else
            ...
            If EstadoMuestraCalc = "CA" Or EstadoMuestraCalc = "CE" Then
                ...
                Exit Sub
            End If
            Mensaje = Calcular.CalcularMuestras("MUESTRAS", False, , , MuestraCalc)
            Seguridad.RegistroAuditoria(Session("Usuario"), "Calcular", "Individual", "FechaIn:" & Me.FECHAIN.Text & ";FechaFin:" & Me.FECHAFIN.Text & ";Muestra:" & Me.MuestraCalcular.Text.Trim & ";Tabla:Muestras", Session("GRUPOUS"))
        End If
        CargarGridView()
        Me.mensaje.ForeColor = Color.Blue
        ...
        <table>
            ...
            <tr>
                <td align="left" colspan="4" height="1">
                    <asp:GridView ID="MUESTRAS" runat="server" Caption="Análisis de Laboratorio" CellPadding="3" ForeColor="#336677" Font-Overline="False" OnRowCommand="MUESTRAS_RowCommand" BorderColor="#CCCCCC" BorderWidth="1px" BackColor="White" BorderStyle="None" Height="41px" Width="166px">
